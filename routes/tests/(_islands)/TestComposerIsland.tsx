...
export default function TestComposerIsland({ testId, nodeId }: TestComposerIslandProps) {
  ...
  const [chatMessages, setChatMessages] = useState<Array<{ role: "user" | "assistant"; content: string | string[] }>>([]);
  ...
  useEffect(() => {
    ...
    if (testId) {
      const foundTest = testStore.tests.value.find(t => t.id === testId) || null;
  ...
  const handleQuestionChange = (index: number, field: string, value: string | string[] | number) => {
    ...
    if (field === "type") {
      ...
      const newType = value as "multiple_choice" | "true_false" | "short_answer";
  ...
  const handleCorrectAnswerChange = (questionIndex: number, value: string | number) => {
    if (!test) return;
  ...
  const removeImage = (questionIndex: number) => {
    ...
    if (fileInputRefs.current[`question-${questionIndex}`]) {
      (fileInputRefs.current[`question-${questionIndex}`] as HTMLInputElement).value = "";
    }
  };
  ...
  const processAIResponse = (response: string) => {
    try {
      ...
      for (const match of jsonMatches) {
        ...
        if (parsedData.type === "test" && parsedData.questions && parsedData.questions.length > 0) {
          ...
          if (confirm(`Add ${newQuestions.length} questions from the AI response to your test?`)) {
            setTest(prev => {
              if (!prev) return null;
              return {
                ...prev,
            ...
            setSuccessMessage(`Added ${newQuestions.length} questions to your test!`);
          ...
          if (confirm(`Add the following question to your test?\n\n${parsedData.question}`)) {
            setTest(prev => {
              if (!prev) return null;
              return {
                ...prev,
  ...
  const handleChatSubmit = async () => {
    ...
    try {
      ...
      if (test) {
        testContext = `I'm currently working on a test titled "${test.name}" with ${test.questions.length} questions.`;
      }
...
\`\`\``;
  ...
  const generateQuestionsWithAI = async () => {
    ...
    try {
      ...
      let prompt = `Generate ${test.questions.length > 0 ? 'additional' : 'new'} test questions for a test titled "${test.name}"`;
      ...
      prompt += `. Please create 3 varied questions with a mix of multiple choice, true/false, and short answer types.`;
...
\`\`\``;
      ...
      setSuccessMessage("AI has generated questions! Review them in the chat and add them to your test.");
  ...
  if (!test) {
    return <div class="container mx-auto px-6 py-10">Failed to load test data</div>;
  }
  ...
  return (
    <div class="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div class="bg-white rounded-lg shadow-md p-6 mb-8">
        <h1 class="text-3xl font-bold mb-6">{testId ? "Edit Test" : "Create New Test"}</h1>
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-4">
            <div class="mb-4">
              <label htmlFor="test-name" class="block text-sm font-medium text-gray-700 mb-1">
                Test Name
              </label>
              <input
                id="test-name"
            ...
            <div>
              <label htmlFor="test-node" class="block text-sm font-medium text-gray-700 mb-1">
            ...
            <div>
              <label htmlFor="test-description" class="block text-sm font-medium text-gray-700 mb-1">
                Test Description
              </label>
              <textarea
                id="test-description"
            ...
            {test.questions.length === 0 ? (
              ...
              <div class="space-y-8">
                {test.questions.map((question, questionIndex) => (
                  <div key={question.id} class="border border-gray-200 rounded-lg p-6 bg-gray-50">
                    ...
                    <div class="space-y-4">
                      <div>
                        <label htmlFor={`question-${questionIndex}-text`} class="block text-sm font-medium text-gray-700 mb-1">
                          Question Text
                        </label>
                        <textarea
                          ...
                          onInput={(e) => handleQuestionChange(questionIndex, "question", (e.target as HTMLTextAreaElement).value)}
                          required
                        />
                      </div>
                      ...
                      <div class="border border-dashed border-gray-300 rounded-lg p-4">
                        ...
                        <input
                          ...
                          accept="image/*"
                          class="hidden"
                          ref={(el) => fileInputRefs.current[`question-${questionIndex}`] = el}
                      ...
                      <div>
                        <label htmlFor={`question-${questionIndex}-type`} class="block text-sm font-medium text-gray-700 mb-1">
                          Question Type
                        </label>
                        <select
                      ...
                      {question.type === "short_answer" && (
                        <div>
                          <label htmlFor={`question-${questionIndex}-answer`} class="block text-sm font-medium text-gray-700 mb-1">
                            Correct Answer
                          </label>
                          <textarea
                            ...
                            onInput={(e) => handleCorrectAnswerChange(questionIndex, (e.target as HTMLTextAreaElement).value)}
                          />
                        </div>
                      )}
                    </div>

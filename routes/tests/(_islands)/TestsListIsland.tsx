...
function exportToGoogleDocs(test: Test, nodeName: string) {
  ...
  alert('Test content copied to clipboard. Please paste (Ctrl+V or Cmd+V) into the new Google Doc that opened.');
}
...
function formatTestForHTMLExport(test: Test, nodeName: string): string {
  ...
  let htmlContent = `
    ...
    <body>
      ...
      <div class="metadata">
        ...
        <p><strong>Last Updated:</strong> ${formatDate(test.lastUpdatedAt || test.createdAt)}</p>
      </div>
  `;
  ...
  test.questions.forEach((question, index) => {
    ...
    if (question.imageUrl) {
      htmlContent += `
        <div>
          <img src="${question.imageUrl}" alt="Question ${index + 1} visual" class="question-image">
        </div>
      `;
    }
    ...
    if (question.type === 'multiple_choice' && question.options) {
      ...
      if (typeof question.correctAnswer === 'number' && question.options) {
        const answerLetter = String.fromCharCode(65 + question.correctAnswer);
        htmlContent += `${answerLetter}. ${question.options[question.correctAnswer]}`;
      } else if (Array.isArray(question.correctAnswer)) {
        const answerLetters = question.correctAnswer.map(
          (ans) => typeof ans === 'number' ? String.fromCharCode(65 + ans) : ans
        );
...
export default function TestsListIsland() {
  ...
  const handleEditTest = (e: Event, testId: string) => {
    e.stopPropagation(); // Prevent triggering the parent click handler
    window.location.href = `/tests/compose/${testId}`;
  };
  ...
  const handleExportToDocument = (e: Event, test: Test) => {
    ...
    try {
      ...
      alert('An error occurred while opening the test document. Please try again.');
    }
  };
  ...
  return (
    <div class="container mx-auto px-6 py-8 max-w-4xl">
      ...
      {tests.length === 0 ? (
        <div class="my-8">
          <p class="text-lg text-gray-700 mb-6">No tests have been created yet. Select a node in the graph to create a test or create one directly.</p>

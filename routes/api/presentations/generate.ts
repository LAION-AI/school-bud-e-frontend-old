      ...
      const stream = new ReadableStream({
        async start(controller) {
          try {
            ...
            const prompt = `Create a PowerPoint presentation about "${data.topic}". 
...
Return a well-structured JSON object following the format above. The presentation should be informative, engaging, and suitable for a general audience.`;
            ...
            const response = await fetch(`${apiUrl}/v1/chat/completions`, {
              ...
              body: JSON.stringify({
                ...
                messages: [
                  {
                    role: "system",
                    content: "You are a helpful assistant that creates well-structured PowerPoint presentations. You always respond with valid JSON in the requested format."
                  },
                  {
                    role: "user",
                    content: prompt
            ...
            try {
              ...
              if (!validationResult.success) {
                throw new Error(`Invalid presentation data: ${validationResult.error.message}`);
              }
              ...
              throw new Error(`Failed to extract presentation data: ${errorMessage}`);
            }

...
export default function ConfigurationSelector({
  ...
  useEffect(() => {
    if (savedConfigs.length > 0 && !selectedConfig) {
      ...
      const savedSelectedName = localStorage.getItem(
        `${serviceType}SelectedConfig`,
      );
      const defaultConfig = savedConfigs.find((config) =>
        config.name === savedSelectedName
      ) || savedConfigs[0];
        ...
        {(showNewConfig || savedConfigs.length === 0) && (
          <div class="space-y-4">
            <input
              ...
              value={newConfigName}
              onChange={(e) =>
                setNewConfigName((e.target as HTMLInputElement).value)}

...
export default function PresentationPreviewIsland({ id }: PresentationPreviewIslandProps) {
      ...
      {isLoading ? (
        ...
        <div>
                ...
                {presentationData.slides.map((slide, slideIndex) => (
                  <div 
                    ...
                    class={`absolute top-0 left-0 w-full h-full p-8 transition-opacity duration-300 ${
                      slideIndex === activeSlide ? 'opacity-100 z-10' : 'opacity-0 z-0'
                    }`}
                  >
                    <div class="h-full flex flex-col">
                      {/* Slide title */}
                      <h3 class="text-3xl font-bold mb-6 text-center">{slide.title}</h3>
                        ...
                        <div class="flex-1 flex items-center">
                          <ul class="list-disc pl-8 space-y-4 text-xl">
                            {slide.content.map((point, pointIndex) => (
                              <li key={`point-${slide.title}-${pointIndex}`} class="text-gray-800">{point}</li>
                            ))}
                          </ul>
                        </div>
                          ...
                          <div class="w-2/5 flex-shrink-0 flex items-center justify-center">
                            {isImageUrl(slide.imageUrl) ? (
                              <img 
                                ...
                                onError={(e) => {
                                  ...
                                  if (container) {
                                    ...
                                    placeholder.className = 'w-full h-64 flex items-center justify-center bg-gray-100 rounded shadow-md';
                              ...
                              <div class="w-full h-64 flex items-center justify-center bg-gray-100 rounded shadow-md p-4 text-center">
                                <div>
                                  <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-gray-400 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                  </svg>
                                  <p class="text-sm text-gray-600">Image suggestion: {slide.imageUrl}</p>
                                </div>
                              </div>
                            )}
                          </div>
          ...
          {presentationData.slides[activeSlide]?.notes && (
            <div class="mt-6 p-4 bg-amber-50 rounded-lg border border-amber-100">
              <h4 class="font-medium text-amber-800 mb-2">Presenter Notes:</h4>
              <p class="text-amber-700">{presentationData.slides[activeSlide].notes}</p>
            </div>
          )}
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {presentationData.slides.map((slide, slideIndex) => (
                ...
                >
                  ...
                  <p class="text-xs text-center text-gray-600">Slide {slideIndex + 1}</p>
                </button>
              ))}
            </div>
          </div>
        ...
        <div class="text-center py-12">
          ...
          <p class="text-gray-700">
            {error || "This presentation is not available. It may have been deleted or the ID is invalid."}
          </p>
          <p class="mt-4">
            <a 
              href="/presentations" 

...
export default function ChatModeSelector(props: ChatModeSelectorProps) {
  ...
  const handleModeKeyDown = (
    e: KeyboardEvent,
    modes: ChatMode[] | SearchSubMode[],
  ) => {
    const focusArray = modes === CHAT_MODES
      ? keyboardFocusIndex
      : keyboardSubmodeFocusIndex;
  ...
  const currentMode =
    CHAT_MODES.find((mode) => mode.id === props.selectedMode) || CHAT_MODES[0];
  const currentSubMode = currentModeHasSubmodes.value
    ? SEARCH_SUBMODES.find((submode) => submode.id === props.selectedSubMode) ||
      SEARCH_SUBMODES[0]
    : undefined;

    ...
    for (const file of filesCopy) {
      ...
      if (file.pdf_url?.url) {
        ...
        if (file.pdf_url.size && file.pdf_url.size > 5000000) {
          console.log(
            `[ChatInput] Large PDF detected (${file.pdf_url.size} bytes), marked for truncation`,
          );
        }
        ...
        console.log(
          `[ChatInput] PDF URL: ${file.pdf_url.url.substring(0, 50)}...`,
        );
      }

      if (file.image_url?.url) {
        console.log(
          `[ChatInput] Image URL: ${file.image_url.url.substring(0, 50)}...`,
        );
      }
    }
  ...
  const handleStartStream = async (text = "", transcription?: string) => {
    ...
    if (hasPendingTranscriptions) {
      ...
      addMessage({
        role: "assistant",
        content:
          "❌ Please wait for all PDF transcriptions to complete before sending your message.",
      });
  ...
  return (
    <>
      <div class="max-w-4xl w-full mx-auto relative">
          ...
          <div class="flex justify-center">
            <div class="p-2 flex flex-wrap max-w-xs gap-8">
              ...
              {files.value
                ...
                .map((pdf, index) => {
                  const isPdfTranscribing = "pdf_url" in pdf &&
                    pdf.pdf_url?.isTranscribing === true;
                  ...
                  return (
                    <button
                      ...
                      class={`w-32 h-32 relative group rounded-lg shadow-xl overflow-hidden cursor-pointer grid place-content-center bg-white ${
                        isPdfTranscribing ? "ring-2 ring-primary-500" : ""
                      }`}
                        ...
                        <div class="absolute inset-0 bg-white/70 flex items-center justify-center">
                          <div class="flex flex-col items-center">
                            <IconLoader2
                              ...
                              size={28}
                            />
                            <span class="text-xs text-primary-600 text-center">
                              Transcribing...
                            </span>
                          </div>
                        </div>
        ...
        <div
          className={`flex cursor-text flex-col rounded-xl px-4 py-2 shadow-[0_2px_6px_rgba(0,0,0,0.1)] transition-colors bg-white mx-4 mb-4 ${

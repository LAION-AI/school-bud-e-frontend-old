...
export function isFlashcardsJson(data: BaseFormat): data is FlashcardsJson {
  return data.type === "flashcards" &&
    Array.isArray((data as FlashcardsJson).cards);
}
...
export function isPresentationJson(data: BaseFormat): data is PresentationJson {
  return data.type === "presentation" &&
    Array.isArray((data as PresentationJson).slides);
}
 ...
 */
export function extractFormattedData<
  T extends SupportedFormat = SupportedFormat,
>(
  text: string,
  ...
  try {
    ...
    try {
      ...
      const error = `Failed to parse JSON: ${
    ...
    if (options.expectedType && parsedData.type !== options.expectedType) {
      const error =
        `Expected format type "${options.expectedType}" but got "${parsedData.type}"`;
      if (options.throwOnError) {
        throw new Error(error);
      }
      return { success: false, error, rawJson: jsonStr };

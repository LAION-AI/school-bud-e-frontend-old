...
type ShepherdTour = Shepherd.Tour;
...
export function markTourComplete(tourId: string) {
  if (!completedTours.value.includes(tourId)) {
    ...
    if (isBrowser) {
      localStorage.setItem('shepherd-completed-tours', JSON.stringify(completedTours.value));
      ...
      globalThis.dispatchEvent(new CustomEvent('tour:complete', { detail: { tourId } }));
    }
  }
}
...
export function saveTourState() {
  ...
  if (activeTourId.value) {
    ...
    localStorage.setItem('shepherd-current-step-index', currentStepIndex.value.toString());
...
export function initTourGuide() {
  ...
  if (completedToursJson) {
    try {
      const completedTours = JSON.parse(completedToursJson);
...
// Get Shepherd from globalThis in browser environments
const getShepherd = () => {
  if (isBrowser) {
    return (globalThis as any).Shepherd;
  }
  return null;
};
...
function setupGraphTour(): ShepherdTour | undefined {
  ...
  tour.addStep({
    id: 'node-actions',
    title: 'Node Actions',
    text: 'When a node is selected, you can add, rename, delete, or perform other actions using these buttons.',
    attachTo: {
  ...
  tour.addStep({
    id: 'graph-finish',
    title: 'All Set!',
    text: 'You now know how to work with the knowledge graph. Try creating and connecting some nodes!',
    attachTo: {
      element: '.app-container',
      on: 'bottom'
    },

...
export function loadSavedGraphs() {
  ...
  if (savedGraphsJson) {
    try {
      const savedGraphs = JSON.parse(savedGraphsJson) as Record<
        string,
        GraphJson
      >;
...
export function saveGraphs() {
  ...
  const graphsObj = Array.from(graphs.value.entries()).reduce(
    (obj, [key, value]) => {
      obj[key] = value;
      return obj;
    },
...
export function loadGraph(id: string) {
  ...
  if (graph) {
    ...
    console.log(
      `Graph loaded with ${graphCopy.items.length} items:`,
      graphCopy,
    );
...
export function handleConnectNodes() {
  console.log("Connect");
  if (
    !selectedNode.value || !secondarySelectedNode.value ||
    !isConnectingNodes.value
  ) return;

  const updatedGraph = { ...graphData.value || { type: "graph", items: [] } };
  const sourceNode = updatedGraph.items.find((item) =>
    item.item === selectedNode.value
  );
  const targetNode = updatedGraph.items.find((item) =>
    item.item === secondarySelectedNode.value
  );
  ...
  if (sourceNode && targetNode) {
    ...
    const connectionExists = sourceNode.connections.some(
      (conn) =>
        (conn.from === selectedNode.value &&
          conn.to === secondarySelectedNode.value) ||
        (conn.from === secondarySelectedNode.value &&
          conn.to === selectedNode.value),
